...
int	is_redir_ahead(const t_token_stream *ts)
{
    return (ts_check(ts, TOK_REDIR_IN)
        ...
        || ts_check(ts, TOK_REDIR_0_IN)
        || ts_check(ts, TOK_REDIR_1_IN)
        || ts_check(ts, TOK_REDIR_2_IN)
        || ts_check(ts, TOK_REDIR_0_OUT)
        || ts_check(ts, TOK_REDIR_1_OUT)
        || ts_check(ts, TOK_REDIR_2_OUT)
